...
> [!prf]
> Ta xác định độ đo của tập $\varphi(T)$ thông qua độ đo của phần bù của nó là tập $\varphi(G)$. Nhắc lại cấu trúc của tập mở $G = \bigcup_{k,j}I_{k,j}$. Trên mỗi khoảng mở ${} I_{j,k}$, hàm $\tau(x)$ nhận giá trị là một hằng số $c_{k,j}$
