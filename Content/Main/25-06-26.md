...
> **Mệnh đề gốc bị lỗi:** Cho $f_n \to f$ a.e. trên $D$ với $\mu(D) = \infty$. Nếu chỉ có điều kiện bị chặn hằng số $f_n(x) \le M$ a.e., dấu bằng giới hạn tích phân sẽ sụp đổ (Phản ví dụ Prob 8.10: $f_n = \frac{1}{n}\chi_{[1, n]} \to 0$ a.e. nhưng $\lim \int f_n = 1 \neq 0$).
> 
> Để mệnh đề hội tụ bị chặn trên chạy được, ta có 2 phương án sửa đổi độc lập tương ứng với 2 định lý lớn:
> 
> Phương án 1: Trói buộc cấu trúc Đơn điệu (Vận dụng Generalized MCT — Thm 9.17)
> * **Giả thiết bổ sung:** Ép dãy hàm chạy theo một chiều cố định: **Tính đơn điệu tăng** ($f_n \uparrow f$ a.e. trên $D$).
> * **Derivation (Phép quy đổi đại số):** >   Vì $f_n \uparrow f$ a.e. và bị chặn trên bởi hằng số $f_n \le M$, ta trói buộc được ranh giới hai đầu: $f_1(x) \le f_n(x) \le M$ a.e. trên $D$. 
>   Kích hoạt trực tiếp **Generalized MCT (Theorem 9.17(a))** với hàm mốc chặn dưới là $g(x) = f_1(x)$ (chú ý: nếu $f_1$ chưa khả tích, ta chỉ cần xét từ một số hạng thứ $n_0$ khả tích hoặc dùng kỹ thuật tịnh tiến nền đối với không gian đo hữu hạn), ta lập tức derive được đẳng thức:
>   $$\lim_{n \to \infty} \int_D f_n \, d\mu = \int_D f \, d\mu$$
> 
> Phương án 2: Trói buộc bằng Tích phân toàn cục (Vận dụng Generalized DCT — Prob 9.22)
> * **Giả thiết bổ sung:** Cho phép hàm nhấp nhô tự do (không đơn điệu), nhưng ép **Giới hạn tích phân toàn cục phải hội tụ**: $\lim_{n \to \infty} \int_D f_n \, d\mu = I < \infty$.
> * **Derivation (Phép quy đổi đại số):** >   Mục tiêu là chứng minh thuộc tính bảo toàn tích phân di truyền xuống **mọi tập con $E \subset D$** theo tinh thần của bài toán tổng quát. 
...
> [!def] Định nghĩa: Trực giao với không gian đa thức ($w$-trực giao)
> Cho hàm trọng số $w(x) > 0$ trên khoảng $(a, b)$. Một hàm số $f \in C[a, b]$ không đồng nhất bằng $0$ được gọi là trực giao với không gian đa thức $\Pi_n$ đối với trọng số $w$, ký hiệu là $f \perp_w \Pi_n$, nếu tích trong của $f$ với mọi đa thức $p \in \Pi_n$ đều bằng $0$:
> $$\langle f, p \rangle = \int_a^b f(x)p(x)w(x) dx = 0 \quad \text{với mọi } p(x) \in \Pi_n$$
> 
> *Nhận xét:* Tính chất này bao trùm định nghĩa đa thức trực giao. Theo Bổ đề 1, đa thức trực giao bậc $n$ (ký hiệu $\pi_n$) chính là một trường hợp đặc biệt thỏa mãn $\pi_n \perp_w \Pi_{n-1}$.

> [!cor] Hệ quả: Số lần đổi dấu của hàm trực giao
> Cho hàm số $f \in C[a, b]$ không đồng nhất bằng $0$. Nếu $f \perp_w \Pi_n$, thì hàm $f(x)$ bắt buộc phải đổi dấu ít nhất $n+1$ lần trên khoảng $(a, b)$.

> [!prf]
> Một hàm số liên tục $f(x)$ bất kỳ (không nhất thiết phải là đa thức), nếu nó sở hữu đặc tính hình học tương tự là $f \perp_w \Pi_n$, thì để tích phân toàn cục $\int_a^b f(x)p(x)w(x)dx$ luôn bằng $0$ với mọi đa thức $p \in \Pi_n$, đồ thị của $f(x)$ bắt buộc phải cắt qua trục hoành ít nhất $n+1$ lần trên khoảng $(a, b)$.
