...
# Liên kết Hình học: Hình chiếu Trực giao trên Không gian Hilbert

> [!obs] (Bước chuyển sang Không gian Hilbert $L^2(\Omega, \mathcal{F}, \mathbb{P})$ và Hình chiếu Trực giao)
> 1. **Cấu trúc Hilbert:** 
>    Khi giới hạn phạm vi khảo sát trong không gian các biến ngẫu nhiên có phương sai hữu hạn $L^2(\Omega, \mathcal{F}, \mathbb{P})$, không gian này trở thành một **Không gian Hilbert** với tích vô hướng $\langle X, Y \rangle = \mathbb{E}[XY]$ và chuẩn $\|X\|_2 = \sqrt{\mathbb{E}[X^2]}$.
> 2. **Không gian con đóng Kéo về:** 
>    Với một $\sigma$-đại số con $\mathcal{G} \subseteq \mathcal{F}$ (chẳng hạn $\mathcal{G} = \sigma(\mathbf{Z}) = \mathbf{Z}^*(\mathcal{B}(\mathbb{R}^k))$), tập hợp các biến ngẫu nhiên $\mathcal{G}$-đo được có phương sai hữu hạn $L^2(\Omega, \mathcal{G}, \mathbb{P}|_{\mathcal{G}})$ tạo thành một **không gian con đóng (closed subspace)** của $L^2(\Omega, \mathcal{F}, \mathbb{P})$.
> 3. **Đặc trưng Hình chiếu Hilbert:** 
>    Theo **Định lý Chiếu Trực giao (Orthogonal Projection Theorem)** trên không gian Hilbert, với mọi $X \in L^2(\mathcal{F})$, tồn tại duy nhất một phần tử $\hat{X} \in L^2(\mathcal{G})$ sao cho vectơ sai số $X - \hat{X}$ trực giao với toàn bộ không gian con $L^2(\mathcal{G})$:
>    $$\langle X - \hat{X}, Z \rangle = \mathbb{E}[(X - \hat{X})Z] = 0, \quad \forall Z \in L^2(\mathcal{G}).$$
>    Đặc biệt, chọn hàm thử $Z = \mathbb{1}_G$ với $G \in \mathcal{G}$, điều kiện trực giao trở thành:
>    $$\int_G \hat{X} \, d\mathbb{P} = \int_G X \, d\mathbb{P}, \quad \forall G \in \mathcal{G}.$$
>    Kết luận: **Kỳ vọng điều kiện $\mathbb{E}[X \mid \mathcal{G}]$ trong $L^2$ chính là Hình chiếu Trực giao của vectơ $X$ lên không gian con đóng $L^2(\mathcal{G})$**:
>    $$\mathbb{E}[X \mid \mathcal{G}] = \text{Proj}_{L^2(\mathcal{G})}(X).$$
> 4. **Mối liên hệ tối ưu hóa Hồi quy:** 
>    Kết hợp góc nhìn hình chiếu Hilbert với định lý biểu diễn Doob–Dynkin $\mathbb{E}[X \mid \mathbf{Z}] = g(\mathbf{Z})$, hàm hồi quy $g(\mathbf{Z})$ chính là nghiệm duy nhất của bài toán tối ưu hóa sai số phương sai trung bình nhỏ nhất (MMSE - Minimum Mean Square Error) trong lớp các hàm Borel kéo về:
>    $$g = \arg\min_{h \text{ Borel}} \mathbb{E} \left[ |X - h(\mathbf{Z})|^2 \right].$$
