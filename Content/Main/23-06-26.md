...
> 
> Áp dụng tính bán cộng tính đếm được (countable subadditivity) của độ đo $\mu$ lên hệ thức hợp trên, ta có bất đẳng thức:
...
> 
> Sự mâu thuẫn này chứng tỏ giả thiết phản chứng ban đầu là sai. Ta kết luận $\mu(\{x \in D : f(x) > 0\}) = 0$, nghĩa là $f = 0$ hầu khắp nơi (a.e.) trên $D$. $\blacksquare$
